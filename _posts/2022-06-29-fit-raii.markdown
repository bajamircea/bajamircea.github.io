...
`operator->` don't usually make sense for handles. `handle_reference` and
`handle_pointer` are provided for direct (dangerous, but useful sometimes)
...
equal values will be those of invalid handles.
...
`open(read(...)...);` on a single line using the file functions below, instead
