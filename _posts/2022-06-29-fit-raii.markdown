...
The operator `bool` checks for `is_valid()`, it is `explicit`, limiting its
use e.g. to `if` tests.
...
choice that its consructor does not accept a temporary RAII class, because
...
wrapper for `FILE *`. Its single responsibility is to ensure that a valid
...
It is not the traditional OOP approach, which surprises some, but it has its
advantages.
...
Its responsibility is to handle exceptions at a boundary (e.g. main in this
